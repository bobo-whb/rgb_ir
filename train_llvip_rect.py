...
class RGBIRDetDataset(Dataset):
    ...
    def _spectral_augment(self, rgb, ir):
        ...
        beta = random.uniform(0.0, self.spectral_strength * 0.5)
        rgb_f = rgb.astype(np.float32)
        ir_f = ir.astype(np.float32)
        rgb_mix = np.clip(rgb_f * (1.0 - alpha) + ir_f * alpha, 0, 255).astype(np.uint8)
        ir_mix = np.clip(ir_f * (1.0 - beta) + rgb_f * beta, 0, 255).astype(np.uint8)
        return rgb_mix, ir_mix
...
def get_args():
    ...
    ap.add_argument("--logdir",    type=str, default="./runs/1224_dualaug")
