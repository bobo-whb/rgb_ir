...
class RGBIRDetDataset(Dataset):
    ...
    带数据增强版本：
      - augment=True 时：
          ...
          * 随机翻转
          * letterbox 随机缩放抖动
      - 增强都作用在原始坐标系下的旋转框 (cx,cy,w,h,angle)，
        然后再统一做 letterbox 到 imgsz。
    """
    def __init__(self, csv_path: str, imgsz: int = 640, augment: bool = True,
                 scale_jitter_range: Tuple[float, float] = (0.75, 1.0)):
        super().__init__()
    ...
    def __getitem__(self, idx: int):
        ...
        # ---------- 数据增强 ----------
        if self.augment:
            rgb, ir, rboxes, clses = augment_pair(rgb, ir, rboxes, clses)

        # ---------- letterbox 到统一尺寸 ----------
        if self.augment and self.scale_jitter_range is not None:
            lo, hi = self.scale_jitter_range
            if hi < lo:
                lo, hi = hi, lo
            scale_jitter = random.uniform(lo, hi)
        else:
            scale_jitter = 1.0
        rgb, scale, pad = letterbox(rgb, self.imgsz, scale_jitter=scale_jitter)
        ir,  _,    _    = letterbox(ir,  self.imgsz, scale_jitter=scale_jitter)
        ...
        target = {
            "boxes": torch.from_numpy(rboxes_scaled),   # (N,5) [cx,cy,w,h,angle]
            "labels": torch.from_numpy(clses),          # (N,)
            "orig_size": torch.tensor(
                [rgb.size[1], rgb.size[0]], dtype=torch.float32
            ),  # [H, W] after letterbox
        }
        return rgb_t, ir_t, target, os.path.basename(rgb_path)
    
def letterbox(img: Image.Image, new_size: int, scale_jitter: Optional[float] = None) -> Tuple[Image.Image, float, Tuple[float,float]]:
    w, h = img.size
    base = min(new_size / w, new_size / h)
    if scale_jitter is None:
        scale_jitter = 1.0
    scale_jitter = max(scale_jitter, 1e-3)
    scale = base * min(scale_jitter, 1.0)
...
def get_args():
    ...
    ap.add_argument("--val_csv",   type=str, default="../data/val.csv")
    ...
    ap.add_argument("--logdir",    type=str, default="./runs/1201_aug")
